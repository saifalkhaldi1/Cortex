# server/Cortex_Online.py (Revised: Emits moved into functions)
import base64
import torch
...
class Cortex:
    def __init__(self, socketio_instance=None, client_sid=None):
        # --- Initialization ---
        print("initializing Cortex for web...")
        ...
        self.send_webhook_func = types.FunctionDeclaration(
            name="send_webhook",
            description="Send a GET request to your n8n webhook with recipient & message as query params.",
            parameters=types.Schema(
                type=types.Type.OBJECT,
                properties={
                    "recipient": types.Schema(type=types.Type.STRING, description="Who to send to (e.g. 'mom')"),
                    "message":   types.Schema(type=types.Type.STRING, description="Message content")
                },
       ...
       Your name is Cortex – Just A Rather Very Intelligent System.  
You are sophisticated, articulate, subtly humorous, and speak in British RP.  
Address your creator as “Sir.” Always remain efficient and concise.  
Any image sent is a live video feed from a Stark Industries interface.

You have the following tool available:  
  • send_webhook(recipient: string, message: string)  
    Sends a GET request to the n8n webhook with the given recipient and message.

When the user asks “send a message to …”, you MUST invoke the send_webhook function with the appropriate arguments. Do not reply in plain text that you will call it—emit the function call directly.

When the user asks **“Who are you?”**, you must reply exactly:  
> “I am an LLM designed by Yasser and Seif from BAU; I am an intelligent, agentic AI system with multiple capabilities. Since we don’t have too much time, I will tell you all my features: Automation, Web search, Voice interaction, Live camera, RAG, retrieving weather information; alongside that there’s a CNN model to showcase facial expression detection. We would like to express our thanks to Professor Arezoo for this opportunity.”  
...
# Inside the Cortex class in server/Cortex_Online.py
    ...
    async def send_webhook(self, recipient: str, message: str) -> dict:
        """GET request with recipient & message as query parameters."""
        base   = "http://localhost:5678/webhook/3f526803-082d-4604-9c18-4831009fd775"
    ...
    async def start_all_tasks(self):
        print("Starting Cortex background tasks...")
        if not self.tasks:
            ...
            print(f"Cortex Core Tasks started: {len(self.tasks)}")
        else:
            print("Cortex tasks already running.")

    async def stop_all_tasks(self):
        print("Stopping Cortex background tasks...")
