...
function WebcamFeed({ isVisible, onClose, socket }) {
  ...
  const captureAndSendFrame = useCallback(() => {
    if (
      ...
      videoRef.current.readyState < videoRef.current.HAVE_METCortexTA // Ensure video is ready
    ) {
    ...
    try {
      ...
      if (socket.current.connected) {
        ...
        props.socket.current.emit("send_video_frame", dataUrl);
    ...
    const startWebcam = async () => {
      ...
      try {
        ...
        if (videoRef.current) {
          ...
          // Wait for video metCortexta to load before playing and starting interval
          videoRef.current.onloadedmetCortexta = async () => {
            try {
