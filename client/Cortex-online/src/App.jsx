...
function App() {
  ...
  const CortexMessageIndex = useRef(-1);
  ...
  const playNextAudioChunk = useCallback(async () => {
    ...
    setStatusText("Cortex is speaking...");
  ...
  useEffect(() => {
    ...
    const handleError = (event) => {
      ...
      // Set visualizer to idle only if Cortex isn't speaking
    ...
    const handleEnd = () => {
      ...
      if (
        ...
        CortexMessageIndex.current = -1; // Reset index for next Cortex message
        if (socket.current?.connected) {
          socket.current.emit("send_transcribed_text", {
            transcript: processedTranscript,
          });
        }
        setStatusText("Waiting for Cortex..."); // Update status
      } else {
  ...
  useEffect(() => {
    ...
    // Handles incoming text chunks for Cortex's response
    const handleTextChunk = (data) => {
      if (!data || !data.text) return; // Ignore empty chunks
      setMessages((prevMessages) => {
        const newMessages = [...prevMessages];
        // Check if the last message is from Cortex and append, or add new message
        if (
          CortexMessageIndex.current !== -1 && // Ensure index is valid
          CortexMessageIndex.current < newMessages.length && // Bounds check
          newMessages[CortexMessageIndex.current]?.sender === "Cortex"
        ) {
          newMessages[CortexMessageIndex.current].text += data.text;
        } else {
          // Add a new message entry for Cortex
          newMessages.push({ sender: "Cortex", text: data.text });
          CortexMessageIndex.current = newMessages.length - 1; // Update index
        }
        return newMessages;
      });
    };

    // Handles incoming audio chunks for Cortex's speech
  ...
  const handleSendText = useCallback(
    (text) => {
      if (text && socket.current?.connected) {
        ...
        CortexMessageIndex.current = -1; // Reset Cortex message index
